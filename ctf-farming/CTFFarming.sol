...
contract CTFFarm is Ownable {
    ...
    constructor(
        ...
        CTFPerBlock = 6599100000000000; // 0.0065991 CTF per block
    ...
    function pendingCTF(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && tokenSupply != 0) {
            uint256 ctfReward =
                CTFPerBlock.mul(pool.allocPoint).div(
                    totalAllocPoint
                );
    ...
    function withdraw(uint256 _pid, uint256 _amount) public validatePool(_pid) {
        ...
        if (pending > 0) {
            ...
            if(block.number <= startBlock + 30 days) {
                ...
                safeCTFTransfer(teamRewardsReceiver, fee);
                safeCTFTransfer(msg.sender, pending.sub(fee));
    ...
    function updateRewardsPerBlock() internal{
        ...
        if(block.number >= lastChange + timeToChangeRewards) {
            CTFPerBlock = CTFPerBlock.mul(80).div(100);
