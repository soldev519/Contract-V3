...
contract NFTLFarm is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accNFTLPerShare; // Accumulated NFTLs per share, times 1e18.
        uint16 depositFeeBP;      // Deposit fee in basis points , 500 = 5%
    }

    uint256 timeToChangeRewards = 30 days;
    ...
    constructor(
        ...
        NFTLPerBlock = 6599100000000000; // 0.0065991 NFTL per block
        startBlock = _startBlock;
        teamRewardsReceiver = _teamRewardsReceiver;
        lastChange = startBlock;
    }
    ...
    function pendingNFTL(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && tokenSupply != 0) {
            uint256 nftlReward =
                NFTLPerBlock.mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accNFTLPerShare = accNFTLPerShare.add(
                nftlReward.mul(1e18).div(tokenSupply)
            );
        }
        return user.amount.mul(accNFTLPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public validatePool(_pid) {
        ...
        uint256 nftlReward =
            NFTLPerBlock.mul(pool.allocPoint).div(totalAllocPoint);

        // staker share
        uint256 teamReward = nftlReward.mul(teamShare).div(10000);
        
        //if(nftl.balanceOf(address(this)) >= teamReward)
        //    nftl.transfer(teamRewardsReceiver, teamReward); 
    ...
    function deposit(uint256 _pid, uint256 _amount) public validatePool(_pid) {
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accNFTLPerShare).div(1e18).sub(
                    user.rewardDebt
                );
            if (pending > 0) {
        ...
        //user.amount = user.amount.add(_amount);
        user.rewardDebt = user.amount.mul(pool.accNFTLPerShare).div(1e18);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public validatePool(_pid) {
        ...
        if (pending > 0) {
            ...
            if(block.number <= startBlock + 30 days) {
                uint256 fee = pending.mul(teamShare).div(10000);
                
                safeNFTLTransfer(teamRewardsReceiver, fee);
                safeNFTLTransfer(msg.sender, pending.sub(fee));
    ...
    // Safe NFTLs transfer function, just in case if rounding error causes pool to not have enough NFTL.
    ...
    function updateTeamShare(uint256 _newShare) public onlyDev {
        require(_newShare > 0 && _newShare < 10000, "Wrong Values");
        teamShare = _newShare;
    }

    function updateRewardsPerBlock() internal{
        // change rewards every 30 days
        if(block.number >= lastChange + timeToChangeRewards) {
            NFTLPerBlock = NFTLPerBlock.mul(80).div(100);
